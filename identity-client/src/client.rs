...
impl IdentityClient {
    ...
    pub fn get_trust_bundle(
        &self,
    ) -> Box<dyn Future<Item = aziot_identity_common_http::get_trust_bundle::Response, Error = Error> + Send> {
        let client = self.client.clone();
        let uri = format!("/trust-bundle?api-version={}", self.api_version);

        let res = build_request_uri(&self.host, &uri)
            .into_future()
            .and_then(move |uri| request::<_,Value, _>(&client, hyper::Method::GET, &uri, None));
